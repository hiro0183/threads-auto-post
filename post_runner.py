...
# 2026-07-30: 4層ポートフォリオ（拡散→信頼→会話→導線）へ更新。
# 2026-09-11: 導線枠(cta:True)は taboo #5 改定により 3本 → **1日1本**（22:00のみ）。
...
SLOT_PLAN = {
    ...
    "19:30": {"type": "tree", "cta": False, "layer": "信頼"},  # 2026-09-11: CTA解除（導線→信頼）
    "20:00": {"type": "tree", "cta": False, "layer": "会話"},  # 2026-09-11: CTA解除。会話枠=あり方の置き場
    "20:40": {"type": "tree", "cta": False, "layer": "拡散"},
    "21:20": {"type": "tree", "cta": False, "layer": "会話"},
    "22:00": {"type": "tree", "cta": True,  "layer": "導線"},  # 2026-09-11: CTAはこの1枠だけ（taboo #5改定=1日1本）。3枠中この枠が実測中央値最高(12 / 19:30=10 / 20:00=7)
}
...
# 2026-09-11: さらに 3件 → 1件/日 へ。funnel_rules.md の「導線枠1日1本」と正本が矛盾しており、
#   実測で夕方のCTA枠が一桁viewsを量産していたため少ない側へ統一（なりあいさん決定）。
#   SLOT_PLAN の cta=True は 1日1つだけにすること。
